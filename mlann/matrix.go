...
//InsufficientDataError is returned by LoadMatrix if the data provided
//is insufficient to populate the matrix as defined
type InsufficientDataError struct{}

func (e *InsufficientDataError) Error() string {
	return "insufficient data provided to populate the matrix"
}

//LoadMatrix reads matrix data from rdr. The data is expected to be a
//comma separated list of entries rows, columns, data where the length
//of the list is equal to rows * columns + 2. data is parsed into float64
//values.
func LoadMatrix(rdr io.Reader) (*Matrix, error) {
	r := csv.NewReader(rdr)
	record, err := r.Read()
	if err != nil {
		return nil, err
	}
	rows, err := strconv.Atoi(record[0])
	if err != nil {
		return nil, err
	}
	cols, err := strconv.Atoi(record[1])
	if err != nil {
		return nil, err
	}
	if len(record) != rows*cols+2 {
		return nil, &InsufficientDataError{}
	}

	mtx := NewMatrix(rows, cols)
	for idx, item := range record[2:] {
		val, err := strconv.ParseFloat(item, 64)
		if err != nil {
			return nil, err
		}
		mtx.data[idx] = float64(val)
	}
	return mtx, nil
}
...
//Write writes the matrix out as a string appropriate for reading through
//the LoadMatrix function.
func (m *Matrix) Write(w io.Writer) {
	out := fmt.Sprintf("%d,%d", m.Rows(), m.Cols())
	for i := 0; i < len(m.data); i++ {
		out = fmt.Sprintf("%s,%.5f", out, m.data[i])
	}
	w.Write([]byte(out))
}
