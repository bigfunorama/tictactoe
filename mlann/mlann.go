...
func NewMLann(eta, lambda float64) *MLann {
	layers := make([]*Layer, 0)
	return &MLann{layers: layers, eta: eta, lambda: lambda}
}
...
func (m *MLann) Write(w io.Writer) {
	out := fmt.Sprintf("%.5f\n%.5f", m.eta, m.lambda)
	w.Write([]byte(out))
...
func readlayer(rdr *bufio.Reader) (*Layer, error) {
	out := ""
	for idx := 0; idx < 4; idx++ {
	...
	return LoadLayer(strings.NewReader(out))
}

//LoadMLann loads a fully populated network read from r
func LoadMLann(r io.Reader) (*MLann, error) {
	rdr := bufio.NewReader(r)
	data, err := readline(rdr)
	if err != nil {
		return nil, err
	}
	eta, err := strconv.ParseFloat(data, 64)
	if err != nil {
		return nil, err
	}
	...
	lambda, err := strconv.ParseFloat(data, 64)
	if err != nil {
		return nil, err
	}
	mlann := NewMLann(eta, lambda)
	var layer *Layer
	for err == nil {
		layer, err = readlayer(rdr)
		if err != nil {
			if err != io.EOF {
				return nil, err
			}
		} else {
			mlann.AddLayer(layer)
		}
	}
...
//LoadMLannFromFile reads an MLann from the network location and returns it.
func LoadMLannFromFile(network string) (ann *MLann, err error) {
...
//SaveMLannToFile writes the provided network to the file location specified.
func SaveMLannToFile(ann *MLann, networkOut string) error {
	f2, err := os.Create(networkOut)
	defer f2.Close()
	if err != nil {
		return err
	}
